        ...
        usersSnapshot.forEach((doc) => {
          ...
          if (userData) {
            ...
            if (userData.answers) {
              ...
              Object.entries(categoryQuestions).forEach(
                ([category, questions]) => {
                  let totalScore = 0;
                  ...
                  if (answeredQuestions > 0 && domainStats[category]) {
                    // Z 점수 계산
                    const zScore =
                      (scaleScore - domainStats[category].mean) /
                      domainStats[category].sd;
              ...
              averageNewScore =
                categoriesWithScores > 0
                  ? totalNewScore / categoriesWithScores
                  : 0;
              ...
              if (
                userData.mentalHealthHistory &&
                userData.mentalHealthHistory !== "아니오"
              ) {
                riskLevel = "high";
                highRiskPatients++;
    ...
    if (filters.treatmentStatus !== "all") {
      filtered = filtered.filter((patient) => {
        ...
        if (filters.treatmentStatus === "ongoing")
          return patient.currentTreatment.includes("치료 중");
        if (filters.treatmentStatus === "completed")
          return patient.currentTreatment.includes("치료 완료");
        if (filters.treatmentStatus === "recurrence")
          return patient.currentTreatment.includes("재발");
