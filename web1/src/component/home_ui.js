...
export const InfoCard = ({ icon, title, description }) => (
  <Paper elevation={3} sx={{ 
    ...
    <Box fontSize={{ xs: '1.8rem', sm: '3rem' }} mb={{ xs: 0.2, sm: 1 }} color="primary.main">
      {icon}
    </Box>
    <Typography variant={{ xs: 'body2', sm: 'h6' }} fontWeight="600" color="primary" mb={{ xs: 0.5, sm: 1 }}>
      {title}
    </Typography>
  ...
  <Box
    sx={{ 
      ...
      '& > *': {
        ...
        maxWidth: { xs: 'calc(33.333% - 4px)', sm: 'calc(33.333% - 16px)', md: 'none' }
      }
    }}
  >
    <InfoCard
