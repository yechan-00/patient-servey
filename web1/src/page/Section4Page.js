...
const Section4Page = () => {
  ...
  const done = requiredQuestions.filter((id) => localAnswers[id]).length;
  ...
  const handleNext = () => {
    const missing = requiredQuestions.filter((q) => !localAnswers[q]);
    ...
    navigate("/section5", { state: { name: userName, answers: localAnswers } });
  };
    ...
    >
      ...
      <Paper elevation={3} sx={{ p: 4 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mt: 4 }}>
          <Button
            variant="outlined"
            onClick={() => navigate("/section3", { state: { name: userName } })}
          >
            이전
          </Button>
