...
const Section1Page = () => {
  ...
  const done = requiredQuestions.filter((q) => localAnswers[q]).length;
  ...
  const handleNext = async () => {
    ...
    // Firebase에 사용자 답변 저장
    await saveUserAnswers(userName, localAnswers);

    navigate("/section2", { state: { name: userName, answers: localAnswers } });
  };
